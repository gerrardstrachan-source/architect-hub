...
(defparameter *w0-sequence*
  '(S2 S3 S2 S2 S1 S3 S1 S0 S3 S3 S3 S3 S1 S0 S3 S3 S3 S1 S2 S0
    ...
    S3 S1 S0 S1 S2 S1 S2 S1 S0 S1 S0 S2 S1 S2 S1 S3 S3 S1 S2 S0 S1))
...
(defun w0-write-row (stream entry)
  (format stream "~D,~A,~A,~A,~D,~D,~A,~S,~S,~A,~A~%"
          (getf entry :trial)
          (getf entry :state)
          (getf entry :selected-production)
          (getf entry :action)
          (getf entry :correctness)
          (getf entry :environment-reward)
          (or (getf entry :actr-time-action) 0)
          (getf entry :utility-before)
          (getf entry :utility-after)
          (getf entry :seed)
          (getf entry :production-count-after)))

(defun w0-write-log ()
  (with-open-file (stream *w0-output-file*
    ...
    (dolist (entry (reverse *w0-log*))
      (w0-write-row stream entry))))

(defun w0-record-trial (state production action env-reward correctness before-utils)
  (let ((entry (list
                :trial *w0-current-trial*
                :state state
                :selected-production production
                :action action
                :correctness correctness
                :environment-reward env-reward
                :utility-before before-utils
                :utility-after nil
                :actr-time-action (mp-time-ms)
                :actr-time-reward nil
                :seed *w0-seed*
                :production-count-after nil)))
    (push entry *w0-log*)
...
(defun w0-action (state action production)
  (unless *w0-current-trial*
    (error "W0 action occurred without an active trial."))
  (let* ((before-utils (w0-current-utilities))
    ...
    ;; External reward follows action evaluation; no correct action is supplied before choice.
    (trigger-reward env-reward)
    ;; Give reward propagation its own immediate event, then finish the trial.
...
(defun w0-finish-trial ()
  ...
  (if (< *w0-trial-index* (length *w0-sequence*))
      ...
      (progn
        (setf *w0-current-trial* nil)
        (w0-stop))))

(defun w0-stop ()
  (w0-write-log)
  (format t "W0-COMPLETE trials=~D seed=~D production-count=~D~%" (length *w0-log*) *w0-seed* (w0-production-count))
...
(defun run-w0 (&optional (seed 20260907) (output-file "w0-results.csv"))
  ...
  (setf *w0-seed* seed
        *w0-output-file* output-file
        *w0-trial-index* 1
        *w0-current-trial* 1
        *w0-log* nil
        *w0-selection-counts* (make-hash-table :test #'equal))
  (format t "W0-CONFIG version-check pending seed=~D productions=~D~%" seed (w0-production-count))
